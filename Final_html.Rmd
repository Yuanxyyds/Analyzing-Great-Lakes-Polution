...
```{r, include=TRUE}
data_description <- read.csv("data/Datasummary.csv")
...
average_per_lake <- water_phos %>% group_by(lake, yearmon) %>% 
  summarise(mean_value = round(mean(value),3), n = n())
```
...
average_per_lake %>% ggplot(aes(color = lake)) +
...
mean_lake_table <- data.frame(Lake = c("Lake Ontario","Lake Erie",
                              ...
                              MeanPhosphorousLevel =
                                c(round(mean(water_phos_with_city[water_phos_with_city$lake == "LAKE ONTARIO",]$value), 5),
                                round(mean(water_phos_with_city[water_phos_with_city$lake == "LAKE ERIE",]$value), 5),
                                round(mean(water_phos_with_city[water_phos_with_city$lake == "LAKE HURON",]$value), 5),
                                round(mean(water_phos_with_city[water_phos_with_city$lake == "LAKE SUPERIOR",]$value), 5))
         )
...
plotly_heatmap <- plotly_heatmap %>% 
  layout(
    title = "Figure 4 - Heapmap for Phosphrous Level over time by City"
  )
plotly_heatmap 
```
Figure 4 above shows the Heatmap for Phosphorous Levels over time by City, demonstrating the phosphorous levels of cities near Lake Erie and Lake Ontario from 2000 to the present. 
...
mean_city_table <- data.frame(City = c("Hamilton, ON, Canada","Niagara Falls, NY, USA",
                              ...
                              MeanPhosphorousLevel =
                                c(round(mean(water_phos_with_city[water_phos_with_city$city == "Hamilton, ON, Canada",]$value),5),
                                round(mean(water_phos_with_city[water_phos_with_city$city == "Niagara Falls, NY, USA",]$value), 5),
                                round(mean(water_phos_with_city[water_phos_with_city$city == "Toronto, ON, Canada",]$value),5),
                                round(mean(water_phos_with_city[water_phos_with_city$city == "Toledo, OH, USA",]$value),5),
                                round(mean(water_phos_with_city[water_phos_with_city$city == "Sandusky, OH, USA",]$value),5),
                                round(mean(water_phos_with_city[water_phos_with_city$city == "Cleveland, OH, USA",]$value),5)
                               )
         )
...
```
<br>
Based on the summary statistics for mean phosphorous level in the selected cities presented in Table 5 and the Heatmap in Figure 4, it is evident that, with the exception of Toronto, all other cities have significantly higher phosphorous levels than the average value for their respective lakes in Table 4. Moreover, these cities consistently have darker colors in most time periods in <a href='#lf2'>Leaflet 2</a>, indicating a higher level of pollution. Therefore, we can conclude that the **west coasts of Lake Erie and Lake Ontario are the most polluted areas from 2000 to present**. Including the following cities: Hamilton, ON, Canada; Niagara Falls, NY, USA; Toledo, OH, USA; Sandusky, OH, USA; Detroit Lake, OH; and Cleveland, OH, USA.
...
ggplot(interst_city, aes(x = yearmon, y = value, color = city)) +
  ...
  labs(title = "Figure 5 - Time series for phosphorous value of 6 most-polluted cities with Linear fit", x = "Lake", y = "Year")
```
<br>
...
coefficient_summary <- data.frame(City = c("Hamilton, ON, Canada","Niagara Falls, NY, USA",
                    "Toronto, ON, Canada","Toledo, OH, USA",
                    "Sandusky, OH, USA","Cleveland, OH, USA"),
           Coefficient = c(round(coef(modelHamil)[2],8), round(coef(modelNia)[2], 8), 
                           round(coef(modelTor)[2], 8), round(coef(modelTol)[2], 8),
                           round(coef(modelSan)[2], 8), round(coef(modelCle)[2], 8)),
           Pvalue = c(round(coef(summary(modelHamil))[, "Pr(>|t|)"][2], 5), round(coef(summary(modelNia))[, "Pr(>|t|)"][2], 5), 
                           round(coef(summary(modelTor))[, "Pr(>|t|)"][2],5), round(coef(summary(modelTol))[, "Pr(>|t|)"][2], 5),
                           round(coef(summary(modelSan))[, "Pr(>|t|)"][2], 5), round(coef(summary(modelCle))[, "Pr(>|t|)"][2], 5))
             )
```
